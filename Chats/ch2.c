...
void* transmitThread(void * pdata){
	...
	int mapTx_fd = shm_open("mapRx", O_RDWR, 0666); 
	void* mapTx_ptr = mmap(0, 4096, PROT_WRITE, MAP_SHARED, mapTx_fd, 0);
    sem_t *semTx = sem_open("semRx",O_RDWR, 0666, 0);

    char * status;
    int semTxValue;
    
    printf("Init client 2 Tx\n");
